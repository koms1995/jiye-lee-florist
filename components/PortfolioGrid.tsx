...
export default function PortfolioGrid({ images, onProfileClick, onImageClick, activeProfileSi }: Props) {
  ...
  useEffect(() => {
    function init() {
      ...
      // (the sand / vermilion theme). midCopy must satisfy
      // midCopy % N_COLORS === 2 so the wrap-aligned hero override images
      // also land on this colorway every cycle. N_COLORS=6 → midCopy=8.
      //
      ...
      const midCopy    = 8   // 8 % N_COLORS(6) === 2 → colorway 2 (sand)
